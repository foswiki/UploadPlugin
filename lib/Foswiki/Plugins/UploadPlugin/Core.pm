...
sub handleUploadForm {
    ...
    foreach my $key ( 'onsuccess', 'onerror' ) {
        ...
        push @metas,
          "<meta name=\"foswiki.UploadPlugin.$key\" content=\"$val\">";
    }
    if (@metas) {
        my $header = join( "\n", @metas );
...
sub handleRestUpload {
    ...
    foreach my $fileName ( keys %$uploads ) {
        ...
        unless ( $fileSize && $fileName ) {
            if ( $useAjax eq 'on' ) {
                returnRESTResult( $response, 500,
                    "Zero-sized file upload of '$fileName'" );
            }
            else {
                throw Foswiki::OopsException(
                    'attention',
        ...
        writeDebug(
"useAjax=$useAjax, fileName=$fileName, origName=$origName, tmpFileName=$tmpFileName "
        );
        ...
        unless ( defined $fileComment ) {
            writeDebug(
                "filecomment$id not found in query ... reading from topic");
